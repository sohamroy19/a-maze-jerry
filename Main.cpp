...
void setWall(int dir, char relativeDir) {
	if (relativeDir == 'f');			// do nothing
	else if (relativeDir == 'l') dir--;		// rotate anticlockwise
	else if (relativeDir == 'r') dir++;		// rotate clockwise

	dir = (dir + 4) % 4;				// -1 --> 3 and 4 --> 0

	char absoluteDir = (dir == 0)? 'n' : (dir == 1)? 'e' : (dir == 2)? 's' : 'w';
...
void think() {
	...
	if (wallCount == 3) {				// dead end
		...
		if (!wallFront) {
			...
			u = (dir==1)? x+1 : (dir==3)? x-1 : x;		// take a step
			v = (dir==0)? y+1 : (dir==2)? y-1 : y;		// forwards
			scoreF = score[u][v];
		} if (!wallRight) {
			dir = (facing + 1) % 4;				// dir is right
			u = (dir==1)? x+1 : (dir==3)? x-1 : x;		// take a step
			v = (dir==0)? y+1 : (dir==2)? y-1 : y;		// rightwards
			scoreR = score[u][v];
		} if (!wallLeft) {
			dir = (facing + 3) % 4;				// dir is left
			u = (dir==1)? x+1 : (dir==3)? x-1 : x;		// take a step
			v = (dir==0)? y+1 : (dir==2)? y-1 : y;		// leftwards
			scoreL = score[u][v];
		}
...
void move(char relativeDir) {
	...
	x = (facing==1)? x+1 : (facing==3)? x-1 : x;	// adjust the
	y = (facing==0)? y+1 : (facing==2)? y-1 : y;	// coordinates
}
